...
Let's walk you through the implementation of a new command — `memo`.
...
public class RemarkCommand extends Command {
    ...
    public CommandResult execute(Model model) {
        return new CommandResult("Hello from memo");
    }
}
```
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `AddressBookParser#parseCommand()` to recognize the `memo` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
![Output displayed](../images/add-memo/RemarkHello.png)
...
![The relationship between RemarkCommand and Command](../images/add-memo/CommandInterface.png)
...
public class RemarkCommand extends Command {
     ...
     */
    public RemarkCommand(Index index, String memo) {
    ...
    public CommandResult execute(Model model) throws CommandException {
        throw new CommandException(
                String.format(MESSAGE_ARGUMENTS, index.getOneBased(), memo));
    }
...
Now let’s move on to writing a parser that will extract the index and memo from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.address.logic.parser` package. The class must extend the `Parser` interface.

![The relationship between Parser and RemarkCommandParser](../images/add-memo/ParserInterface.png)
...
This appears to be what we need to get a String of the memo. But what about the Index? Let's take a quick peek at existing `Command` that uses an index to see how it is done.
...
public RemarkCommand parse(String args) throws ParseException {
    ...
    String memo = argMultimap.getValue(PREFIX_REMARK).orElse("");

    return new RemarkCommand(index, memo);
}
```
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of person data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the person’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a memo given to a person.
...
## Add a placeholder element for memo to the UI
...
![$memo shows up in each entry](../images/add-memo/$Remark.png)
...
public PersonCard(Person person, int displayedIndex) {
    //...
    memo.setText(person.getRemark().value);
}
```

![The memo label is bound properly!](../images/add-memo/RemarkBound.png)
...
``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added memo to Person: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed memo from Person: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
    ...
    private String generateSuccessMessage(Person personToEdit) {
        String message = !memo.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, personToEdit);
    }
```

![Congratulations!](../images/add-memo/RemarkComplete.png)
...
Let’s create a test for `RemarkCommand#execute()` to test that adding a memo works. On `IntelliJ IDEA` you can bring up the context menu and choose to `Go To` \> `Test` or use the appropriate keyboard shortcut.

![Using the context menu to jump to tests](../images/add-memo/ContextMenu.png)

Then, create a test for the `execute` method.

![Creating a test for `execute`.](../images/add-memo/CreateTest.png)
